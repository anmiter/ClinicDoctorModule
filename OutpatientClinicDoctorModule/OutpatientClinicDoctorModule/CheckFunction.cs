...
namespace OutpatientClinicDoctorModule
{
    ...
    internal class CheckFunction
    {
        ...
        public static bool IsExistTelephone(string telephone)
        {
            ...
            $@"SELECT COUNT(1) FROM tb_Doctor WHERE Telephone=@telephone";

            sqlCommand.Parameters.AddWithValue("@telephone", telephone);//向SQL命令的参数集合添加参数的名称、值；
        ...
        public static bool IsExistQQEmail(string email)
        {
            ...
            sqlCommand.Connection = sqlConnection;
            sqlCommand.CommandText =
            $@"SELECT COUNT(1) FROM tb_Doctor WHERE QQEmail=@email";
        ...
        /// <param name="Email"></param>
        /// <returns></returns>
        public static bool IsExistDoctorNo(string no)
        {
            SqlConnection sqlConnection = new SqlConnection(); //声明并实例化SQL连接；
            sqlConnection.ConnectionString =
                ConfigurationManager.ConnectionStrings["Sql"].ConnectionString; //配置管理器从配置文件读取连接字符串，并将之赋予SQL连接的连接字符串属性；
            ...
            sqlCommand.Connection = sqlConnection;
            sqlCommand.CommandText =
            $@"SELECT COUNT(1) FROM tb_Doctor WHERE No=@no";

            sqlCommand.Parameters.AddWithValue("@no", no);//向SQL命令的参数集合添加参数的名称、值；
